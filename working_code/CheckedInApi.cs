...
namespace MBTP.Services
{
    public class CheckedInApi
    {
        ...
        public CheckedInApi(IDatabaseConnectionService dbConnectionService)
        {
                        ...
                        foreach (var item in result.data)
                        {
                            ...
                            if (checkedIn.InventoryItems != null && checkedIn.InventoryItems.Count > 0)
                            {
                                ...
                                var tariffIds = checkedIn.TariffsQuoted?
                                    ...
                                    .Select(t => t.Id != null ? t.Id.ToString() : string.Empty)
                                    .ToHashSet() ?? new HashSet<string>();

                                var extraInventoryItems = checkedIn.InventoryItems
                                    .Where(inv =>
                                        inv.InventoryItemId != null &&
